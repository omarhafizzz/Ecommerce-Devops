...
until curl -s http://localhost:8080 > /dev/null 2>&1; do
    echo "Jenkins not ready yet... ($COUNT)"
    sleep 10
    COUNT=$((COUNT+1))
    if [ $COUNT -gt 30 ]; then
...
mv /opt/sonar-scanner-${SONAR_VERSION}-linux-x64 /opt/sonar-scanner 2>/dev/null || true
